...
function mapexcel(c){
 ...
 type: 'piecewise',
 pieces: [
 { min: 9999999, max:6, color: '#A40004' },
 { min: 5, max:5, color: '#ff3d00'},
 { min: 4, max:4, color: '#ff6e40' },
 { min: 3, max:3, color: '#ff9473' },
 { min: 2, max:2, color: '#f9b196' },
 { min: 1, max:1, color: '#fee7d6' } 
 ]
 },
...
submitcheck.addEventListener('click', function () {
    ...
    if (beforecontent == '"入"') {
        ...
        $.when($.ajax({
            ...
            let c = countryname.map((name, i) => ({ name, value: coun1990[i] }))
            mapexcel(c)
        })
    } else {
        boolen = false
